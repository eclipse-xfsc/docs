...
sequenceDiagram
    ...
    participant API
    participant VerificationService
    User ->> + API:POST /verifications/self-descriptions<br>(?verifySchema=...&verifyVPSignature=...&verifyVCSignature=...)
...
The verification of SDs consists of up to four stages, each independently configurable (see <<_verification_profiles>>).
Besides verifying the SD, additional tasks are executed (e.g., parsing, claim extraction).

**Syntactic validation** is always active. The SD is parsed as JSON-LD and validated against the Verifiable Credential data model. Any syntactic issue will raise an error here.
...
**Schema validation ** (when enabled) checks the uploaded Self-Descriptions against a composite schema of type SHACL.
If a Self-Description does not conform to the composite schema, a verification exception is thrown with a message containing the validation report. In that case, the verification process does not proceed, and claims are not extracted.
...
**Cryptographic verification (trust verification)** (when enabled) checks all proofs in the SD: whether the signature, the offered key, and the content match.
