...
| RDF Detector | Classifies uploaded content as RDF or non-RDF based on a configurable MIME type allowlist and content inspection (JSON-LD `@context` pattern matching for `application/json`).
...
classDiagram
    class AssetStore {
        ...
        +changeLifeCycleStatusById(id: String, targetStatus: AssetStatus) void
    ...
    class IriGenerator {
        +generateIri(content: byte[], contentType: String) String
        +extractIriFromRdf(content: String) String
