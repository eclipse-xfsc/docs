...
The Federated Catalogue offers the possibility to verify Self-Descriptions. The verification pipeline consists of four independently configurable stages: syntactic parsing, semantic validation, schema validation (SHACL), and cryptographic signature verification. Each stage can be enabled or disabled via configuration properties.
...
The Graph Store is the interface for interacting with the Graph Database. It receives claims (extracted from Self-Descriptions) and adds them to the graph database. The Graph Database only contains claims from active Self-Descriptions and offers a query interface (openCypher for the Neo4j backend, SPARQL-star for the Fuseki backend). External ontologies are not queried when processing requests. The interface also exposes the backend type and supported query language so that the service layer can validate incoming queries before they reach the database.
...
public interface GraphStore {
     ...
     *
     * @return the supported QueryLanguage, or null if the store is disabled
     */
     ...
     *
     * @return the GraphBackendType of this store
     */
...
flowchart LR
    ...
    style default fill:#e8f5e9,stroke:#4caf50
....
