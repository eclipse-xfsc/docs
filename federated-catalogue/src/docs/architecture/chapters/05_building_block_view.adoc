...
| File Store | The File Store is a blob storage. It holds Schema files and non-RDF assets, including historical versions of Schemas. Credential (RDF) content lives in the Metadata Store instead — see below. Non-RDF assets are stored in a dedicated File Store instance, separate from schema and context caches.
| Metadata Store | Store for metadata on assets (credentials, Schemas, and non-RDF files). Credential (RDF) content is kept in the Metadata Store itself (a raw-content column); Schema and non-RDF file bytes are stored in the File Store.
|===
...
The Graph Database can be considered as a kind of search index. The single source of truth are the active credentials, stored in the Metadata Store. This means at any point in time the Graph database can be rebuilt from scratch by reimporting the claims of the credentials. This allows the following:

. **Backup**: An explicit backup of the Graph database is not needed. Backing up the Metadata Store (which holds credential content directly, plus all asset metadata) and the non-RDF asset File Store is sufficient to allow the rebuild of the Graph Database.
...
==== File Store
The File Store is responsible to persist non-RDF file based content submitted to the catalogue: Schemas and non-RDF assets. Credentials (Verifiable Presentations or standalone Verifiable Credentials) are not persisted here — their content is kept in the Metadata Store instead.
...
| Asset Store | Storing and retrieving of credential files and non-RDF assets, and handling their lifecycle, versioning, and provenance. Credential (RDF) bytes are kept in the Metadata Store's raw-content column; only non-RDF asset bytes go to the File Store. Non-RDF assets bypass verification and graph storage. Provenance credentials (W3C VC JSON-LD or VC-JWT) are stored append-only in a dedicated `provenance_credentials` table via the `ProvenanceService`; the table is not Envers-audited, so provenance writes never produce a new asset revision.
...
For **credentials (RDF)**, the store saves the metadata — including the raw JSON-LD body in the raw-content column — in the Metadata Store, then adds claims to the Graph database. No File Store write occurs for RDF credentials; the content is served back directly from that column. A transaction ensures that the stored data is consistent.
...
* for a trust-framework compliance check that could not reach a verdict (the trust service was unreachable or timed out), a failure category distinguishing that outcome from an actual non-conformance verdict; null for schema validations and for compliance checks that did produce a verdict;
* a content hash over the core fields — including the failure category, when present — computed by `ValidationResultHasher` (SHA-256 over the RFC 8785 canonical JSON representation) to support tamper detection;
* a graph-projection status indicating whether the row was mirrored into the graph store as `fcmeta:` triples (`SYNCED`/`FAILED`), or deliberately excluded from projection because the row is not itself a claim about an asset (`EXCLUDED` — used for failure-category audit entries, so that a graph rebuild cannot reintroduce the ambiguity between "service unreachable" and "asset non-compliant").
...
After a graph backend reset, the rebuilder re-projects `fcmeta:` triples from the relational store for every validation result row except those marked `EXCLUDED`, restoring graph state without re-running verification.
Rows whose projection succeeds are marked synced; rows whose projection fails are marked accordingly for retry; `EXCLUDED` rows are skipped and counted separately, since they were never meant to be projected.
...
Credential MIME types (`application/vc+ld+json`, `application/vc+jwt`, and the presentation variants) enter the credential verification path (semantic, signature, trust-framework stages — see <<_verification_profiles>>); non-credential RDF serializations (Turtle, N-Triples, RDF/XML) enter the claim-extraction-only path.
SHACL schema validation is never invoked at upload for either path; on-demand validation is available via `POST /assets/validate`.
...
Exact property names, defaults, and environment-variable forms are documented in the wiki https://github.com/eclipse-xfsc/federated-catalogue/wiki/Installation-%26-Configuration-Guide[Installation & Configuration Guide].
...
| `TrustFrameworkAdminService` | `TrustFrameworkAdminApiDelegate` | List trust framework families with their bundles; toggle family enablement, per-bundle external client identifiers, and per-base-class enablement within a bundle — all via `PATCH` (`application/merge-patch+json`)
