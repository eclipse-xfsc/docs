...
| Verification | Responsible for applying the defined verification rules on the Self-Descriptions (syntactic, semantic, cryptographic). Also extracts the claims from the Verifiable Presentation (_Claim Extractor_). As specified by CAT-FR-SF-04, automatic schema validation against SHACL shapes has been removed from the upload flow and extracted to a dedicated _Schema Validation Service_. Also contains a component to periodically revalidate submitted Self-Descriptions (_Revalidation Service_).
...
As specified by CAT-FR-SF-04, automatic schema validation has been **removed from the upload verification flow**. The Schema Validation Service was extracted from the `VerificationService` to provide on-demand SHACL schema validation of stored assets. This service prepares the infrastructure for CAT-FR-CO-05, which will expose on-demand validation of stored assets against specific schemas via a dedicated API endpoint.

[source,java]
----
/**
 * Dedicated service for SHACL schema validation of Self-Descriptions.
 * Extracted from VerificationService to support on-demand validation (CAT-FR-CO-05).
 */
@Service
public interface SchemaValidationService {
   ...
   *
   * @param payload The Self-Description to validate.
   * @param schema The SHACL shape graph to validate against.
  ...
  /**
   * Validates a Self-Description payload against the composite schema (union of all stored SHACL shapes).
   *
   * @param payload The Self-Description to validate.
   * @return the result of the schema validation.
   */
  ...
  /**
   * Validates pre-extracted claims against a specific SHACL shape graph.
   *
   * @param claims The extracted claims to validate.
   * @param schema The SHACL shape graph to validate against.
   * @return the result of the schema validation.
   */
  ...
  /**
   * Validates pre-extracted claims against the composite schema (union of all stored SHACL shapes).
   *
   * @param claims The extracted claims to validate.
   * @return the result of the schema validation.
   */
